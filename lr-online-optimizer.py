...
class OnlineLinearRegressionOptimizer(AbstractOptimizer):
    ...
    def __init__(self, design_space):
        ...
        self.x = []
        self.y = []
        print("design space size :")
        print(self.design_space.size)
    ...
    def suggest(self):
        ...
        try:
            ...
            print("here is potential pareto frontier")
            print(potential_parteo_frontier)
            print("here is torch.Tensor(potential_suggest)")
            print(torch.Tensor(potential_suggest))
            _potential_suggest = []
            for point in potential_parteo_frontier:
                index = torch.all(ppa == point.unsqueeze(0), axis=1)
                print("here is point and index")
                print(point)
                print(index)
                ...
                print("what it append before tolist")
                print(torch.Tensor(potential_suggest)[
                        torch.all(ppa == point.unsqueeze(0), axis=1)
                    ])
            print("is _potential_suggest == potential_suggest ?")
            print(_potential_suggest == potential_suggest)
