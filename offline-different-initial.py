...
class OfflineLinearRegressionOptimizer(AbstractOptimizer):
    ...
    def __init__(self, design_space):
        ...
        self.model_random = GaussianProcessRegressor(
            kernel=kernel,
            optimizer="fmin_l_bfgs_b",
            random_state=0
        )
        self.model_oa = GaussianProcessRegressor(
            kernel=kernel,
            optimizer="fmin_l_bfgs_b",
            random_state=0
        )
        self.model_ted = GaussianProcessRegressor(
            kernel=kernel,
            optimizer="fmin_l_bfgs_b",
            random_state=0
        )
    ...
    def suggest(self):
        ...
        if not self.first:
            ...
            # if not self.random:
            #     if self.oa:
            #         initial_suggest = self.orthogonal_array()
            #     else:
            #         ted = [1778, 3407, 815, 3389, 2759, 1796, 2426, 4370, 1859, 3254, 3431, 1826, 4388, 2444, 2741, 797, 1676, 3400, 3507, 158, 5027, 2107, 2976, 2106, 3181]
            #         initial_suggest = [
            #             self.design_space.vec_to_microarchitecture_embedding(
            #                 self.design_space.idx_to_vec(_x_guess)
            #             ) for _x_guess in ted
            ...
            ted = [1778, 3407, 815, 3389, 2759, 1796, 2426, 4370, 1859, 3254, 3431, 1826, 4388, 2444, 2741, 797, 1676, 3400, 3507, 158, 5027, 2107, 2976, 2106, 3181]
            oa_suggest = self.orthogonal_array()
            ted_suggest = [
                        self.design_space.vec_to_microarchitecture_embedding(
                            self.design_space.idx_to_vec(_x_guess)
                        ) for _x_guess in ted
            ...
            self.first = False
            full_suggest = []
            full_suggest.extend(random_suggest)
            full_suggest.extend(oa_suggest)
            full_suggest.extend(ted_suggest)
            full_suggest.extend(testing_suggest)
    ...
    def observe(self, x, y):
        ...
        if self.fit:
            ...
            total_x = np.array(x)
            total_y = np.array(y)
            training_x_random = total_x[:self.training_size + 1]
            training_y_random = total_y[:self.training_size + 1]
            training_x_oa = total_x[self.training_size:2*self.training_size + 1]
            training_y_oa = total_y[self.training_size:2*self.training_size + 1]
            training_x_ted = total_x[2*self.training_size:3*self.training_size + 1]
            training_y_ted = total_y[2*self.training_size:3*self.training_size + 1]
            testing_x = total_x[3*self.training_size:]
            testing_y = total_y[3*self.training_size:]
            self.model_random.fit(training_x_random, training_y_random)
            self.model_oa.fit(training_x_oa,training_y_oa)
            self.model_ted.fit(training_x_ted,training_y_ted)
            pred_ran = self.model_random.predict(testing_x)
            mape_ran = mean_absolute_percentage_error(testing_y, pred_ran)
            pred_oa = self.model_oa.predict(testing_x)
            mape_oa = mean_absolute_percentage_error(testing_y, pred_oa)
            pred_ted = self.model_ted.predict(testing_x)
            mape_ted = mean_absolute_percentage_error(testing_y, pred_ted)
            print("random: %f"%mape_ran)
            print("oa : %f"%mape_oa)
            print("ted : %f"%mape_ted)
            # print(
            #     "linear regression trained on data size: {}, " \
            #     "mean absolute percentage error of random: {}".format(
            #         self.training_size,
            #         mape_ran
            #     )\
            #     "mean absolute percentage error of random: {}".format(
    ...
    def distance_vec(self,vec1,vec2):
        ...
        rbf = math.exp(-0.5*(dist**2)) #0.02 : 2*sigma^2
        return rbf
