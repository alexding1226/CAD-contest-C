...
class GaussianProcessRegressorOptimizer(AbstractOptimizer):
    ...
    def suggest(self):
        ...
        normalize_potential_suggest = [
            (np.array(_potential_suggest) / self.design_space_cand_max) for _potential_suggest in potential_suggest
        ]
        print("normalize potential suggest")
        print(normalize_potential_suggest)
        if not self.first:
            ...
            ppa = torch.Tensor(self.model.predict(np.array(normalize_potential_suggest)))
    ...
    def observe(self, x, y):
        ...
        for _x in x:
            self.x.append(_x)
            self.normalize_x.append(np.array(_x)/self.design_space_cand_max)
        for _y in y:
            self.y.append(_y)
        self.model.fit(np.array(self.normalize_x), np.array(self.y))
